...
void ADS1115Component::setup() {
    ...
    if (!this->read_byte_16(ADS1115_REGISTER_CONVERSION, &value)) {
        this->mark_failed();
        printf("[E][ADS1115Component] I2C reading error ^^^^^^^^^^^^^^^^^\n");
        return;
    }
    uint16_t config = 0;
    // Clear single-shot bit        Operational status or singleshot conversion start.
    //                              Write 1:Start a single conversion
    //                              Read 0: Deveice is currently performing a conversion  
    //        0b0xxxxxxxxxxxxxxx
